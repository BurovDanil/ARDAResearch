...
>>>>>>> Stashed changes
	...
	%Write about which are the most common reasons%
		\begin{figure}[H]
		\centering
...
The data displayed in the subsection will be regarding the experiment that was conducted among the IT students. We derived three plots in total which are most relevant to our research paper. Here we have shown the average time of completion in seconds between the two interfaces, the time it took for each participant in our experiment finishing the assignment through each interface.\\
	...
	%Write which are the actual average times%
\begin{figure}[H]
	\centering
...
%Say which is the max time, the min time and the average%
%ON TOP%
...
%Say what is the max time, the min time and the average%
%EXPERIMENt
\begin{figure}[H]
	\centering
...
\end{figure}
	...
	When it comes to the experiment the only problem that we believe may limit the final result is the structure of the assignment. In fact, although the tasks the candidate had to perform were well documented, some of them were uncertain on what to exactly do, increasing their time of completion. Since half of the candidates started with the GUI and half with the CLI, this shouldn't have influenced the overall outcome of the experiment.Lastly, some of the candidates already had experience on using the CLI for executing other tasks (for example navigating through folders and editing files), making the performance of the assignment easier and faster
